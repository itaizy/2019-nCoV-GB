...
JS = 'return arguments[0].toDataURL("image/png", 1);'
WidthJS = 'arguments[0].style.width="882px";'
HeightJS = 'arguments[0].style.height="480px";'
...
for img in driver.find_elements_by_xpath("//div[@id='Trends']/div[1]//canvas"):
    tag = "images/quanguo"
    ...
    i += 1
i = 0
for img in driver.find_elements_by_xpath("//div[@id='Trends']/div[2]//canvas"):
    tag = "images/hubei"
    im_info = driver.execute_script(JS, img)  #执行js文件得到带图片信息的图片数据
    im_base64 = im_info.split(',')[1]  #拿到base64编码的图片信息
    im_bytes = base64.b64decode(im_base64)  #转为bytes类型
